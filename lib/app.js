...
App.prototype.downloadRepositories = function() {
  console.log("Downloading the relevant repositories.")
  ...
  remoteRepositories.forEach(function(name) {
    helper.logAndExec("  - master@" + name + ": fetching", "git checkout master && git checkout . && git pull origin master", {
      dir: 'repositories/' + name
    })

    helper.logAndExec("  - master@" + name + ": installing packages", "npm install", {
      dir: 'repositories/' + name
    })
  })
}
...
App.prototype.release = function() {
  ...
  if (code === 0) {
    ...
    cvs.releaseVersion(version, 'master', this.program)
    ...
    cvs.releaseVersion(m2Version, 'milestones/2.0.0', this.program)
  } else {
    process.exit(code)
  }
}
...
App.prototype.updateBundle = function(dialect, branch) {
  ...
  if (this.program.force) {
    helper.logAndExec('  - Pushing the changes to Github', "git push && git push --tags", { dir: "repositories/sequelize-" + dialect })
    helper.logAndExec('  - Publishing the new version to NPM', "npm publish .", { dir: "repositories/sequelize-" + dialect })
  } else {
    helper.logAndExec('  ~ Pushing the changes to Github (fake)', "echo 1", { dir: "repositories/sequelize-" + dialect })
    helper.logAndExec('  ~ Publishing the new version to NPM (fake)', "echo 1", { dir: "repositories/sequelize-" + dialect })
  }
}
